...
> **Step 编号对照**：以下编号是**规划期**的。实际交付时在 Step 1 之后插入了「托盘常驻 + 自安装」（见第 19 章），因此交付阶段的编号整体顺延一位。对外表述与版本号以 [`README.md`](../README.md) 的「状态」一节为准。
>
> | 本章 Step | 交付 Step | 版本 |
> |---|---|---|
> | Step 1 —— 可用版 | Step 1 | `v0.1.x` |
> | 第 19 章 —— 托盘与安装 | Step 2 | `v0.2.x` |
> | Step 2 —— 密码层 | Step 3 | `v0.3.x` |
> | Step 3 —— 自研客户端 | Step 4 | `v0.4.x` |
> | Step 4 —— 运维增强 | Step 5 | `v0.5.x` |
...
**Windows 开始菜单只索引 `.lnk` 文件。** 光把 exe 放进安装目录是不够的——用户按 `Win` 键搜不到任何东西。这是初版安装器漏掉的一环。

生成 `.lnk` 有三种做法，最终选了纯 Go 手工构造：

| 做法 | 问题 |
|---|---|
| 调 PowerShell 的 `WScript.Shell` | 需启动外部进程；企业环境下 PowerShell 可能被 AppLocker 禁用 |
| 引入 `go-ole` 走 COM | 为一个快捷方式引入整套 COM 绑定，与项目的零依赖取向不符 |
| **按 MS-SHLLINK 规范直接写二进制** | 代码多一些，但无依赖、不启进程、可单元测试 |

格式上有两个刻意的选择：

- **路径同时写 ANSI 与 Unicode 两份**。规范允许只写 ANSI，但安装路径含中文时那一份会失真。
- **`StringData` 的前置字段是字符数而非字节数**。含中文时两者不同，写错会截断路径。

`shortcut_test.go` 覆盖头部字段、CLSID、标志位与 LinkInfo 偏移自洽性，并包含一个精确的边界校验：LinkInfo 之后紧跟 StringData，若 `LinkInfoSize` 少算一字节，读到的字符数就会对不上。

### 19.5 单实例保护

快捷方式指向托盘程序，而托盘默认开机自启。没有保护的话，用户从开始菜单点一次就会多出**第二个托盘图标**。

用命名互斥量解决：`Local\YunSSH.Tray`。`Local\` 前缀保证只在当前登录会话内有效，多用户同时登录互不干扰。检测到已有实例时弹一句提示再退出——静默退出会让人以为"点了没反应"。
...
**Linux 托盘：暂缓（2026-09-23）**

Linux 版已以 CLI 形态发布（`for linux/` 的构建与安装脚本），但**不提供托盘**。原因同上：`fyne.io/systray` 在 Linux 依赖 GTK / libayatana-appindicator，会引入 CGO 与动态链接，破坏「单文件静态二进制、scp 到服务器即用」这一核心优势。托盘的工作量约为 CLI 版的 3-5 倍，而服务器场景几乎不需要图形入口；且托盘的全部能力（列出主机、按关键词连接、打开配置）CLI 都已有等价命令。

**Linux 不提供开机自启。** Windows 的自启项登记的是 `ysshtray.exe`——托盘是唯一常驻进程，`yssh.exe` 从不自启。Linux 侧没有任何长期运行的组件，写 systemd 服务只会得到一个没有内容的空服务。「打开终端即可用」由安装脚本的 `--path`（写入 PATH）与 `--completion`（bash 补全）承担，两者都不引入常驻进程。
...
| 项 | 结果 |
|---|---|
| 快捷方式创建 | 安装后 `.lnk` 出现在开始菜单目录，卸载后随之删除 |
| .lnk 结构 | HeaderSize = 0x4C、CLSID 逐字节匹配、LinkFlags = 0xD2、目标路径在 ANSI 与 Unicode 段均正确 |
